...
@MainActor
class SystemCollectorBase: PaneKitCollector, @unchecked Sendable {
    class var SystemBundleID: String {
        fatalError("Subclasses must override SystemBundleID")
    }
    ...
    open override func _collectTabs(for window: PaneKitWindow) async -> [PaneKitWindow] {
        ...
        for (index, el) in tabElements.enumerated() {
            let title = Self.resolveTitle(for: el)
    ...
    internal class func extractTitlesRecursively(from menu: AXUIElement) -> [String] {
        ...
        for item in items {
            ...
            let skip = ["window", "pane", "panel", "workspace", "tool", "preferences", "settings", "hilfe", "menu"]
    ...
    internal class func resolveTitle(for element: AXUIElement) -> String {
        var value: CFTypeRef?
